 ...
 *
 *********************************************************************************/
static esp_err_t _compute_crc(const uint8_t *param_data, int param_len, uint8_t *param_computed_value) {
...
static esp_err_t _log_config(const mjd_sht3x_config_t* param_ptr_config) {
    ...
    ESP_LOGD(TAG, "  i2c_max_ticks_to_wait: %i", param_ptr_config->i2c_max_ticks_to_wait);
 ...
 * _read_register()
 *
 *  @doc To read out the content of one specific register address (3 bytes MSB LSB CRC).
...
static esp_err_t _read_register(const mjd_sht3x_config_t* param_ptr_config, mjd_sht3x_reg_t param_reg,
    ...
    /*
     ...
     f_retval = i2c_master_read_byte(handle, &lsbyte, I2C_MASTER_ACK); // last read must be I2C_MASTER_NACK
     if (f_retval != ESP_OK) {
     ESP_LOGE(TAG, "%s(). ABORT. Receive response i2c_master_read_byte() err %i (%s)", __FUNCTION__, f_retval,
     esp_err_to_name(f_retval));
     i2c_cmd_link_delete(handle);
     ...
     f_retval = i2c_master_read_byte(handle, &crcbyte, I2C_MASTER_NACK); // last read must be I2C_MASTER_NACK
     if (f_retval != ESP_OK) {
     ESP_LOGE(TAG, "%s(). ABORT. Receive response i2c_master_read_byte() err %i (%s)", __FUNCTION__, f_retval,
     esp_err_to_name(f_retval));
     i2c_cmd_link_delete(handle);
    ...
    if (f_retval != ESP_OK) {
        ESP_LOGE(TAG, "%s(). ABORT. Invalid CRC err %i (%s)", __FUNCTION__, f_retval, esp_err_to_name(f_retval));
        // GOTO
        goto cleanup;
    }
    ...
    //   rx_buf[0] = MSB
    //   rx_buf[1] = LSB
    *param_ptr_data = (((uint16_t) rx_buf[0] << 8) | (uint16_t) rx_buf[1]);
