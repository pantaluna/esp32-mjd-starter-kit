...
esp_err_t mjd_string_to_hexstring(const char * param_ptr_input, char * param_ptr_output) {
    ...
    int idx_src;
    int idx_dst;
    for (idx_src = 0, idx_dst = 0; idx_src < strlen(param_ptr_input); idx_src++, idx_dst += 2) {
        sprintf((char*) param_ptr_output + idx_dst, "%02X", param_ptr_input[idx_src]);
    }
    param_ptr_output[idx_dst] = '\0';

    ESP_LOGD(TAG, "mjd_string_to_hexstring() param_ptr_input  len=%i (HEXDUMP)", strlen(param_ptr_input));
    ESP_LOG_BUFFER_HEXDUMP(TAG, param_ptr_input, strlen(param_ptr_input) + 1, ESP_LOG_DEBUG);  // +1 to see the \0
    ESP_LOGD(TAG, "mjd_string_to_hexstring() param_ptr_output len=%i (HEXDUMP)", strlen(param_ptr_output));
    ESP_LOG_BUFFER_HEXDUMP(TAG, param_ptr_output, strlen(param_ptr_output) + 1, ESP_LOG_DEBUG);  // +1 to see the \0
...
esp_err_t mjd_hexstring_to_string(const char * param_ptr_input, char * param_ptr_output) {
    ...
    if (strlen(param_ptr_input) % 2 != 0) {
        ESP_LOGE(TAG, "ABORT. param_ptr_input has an uneven number of characters");
        // GOTO
        goto cleanup;
    }
    ...
    for (idx_src = 0, idx_dst = 0; idx_src < strlen(param_ptr_input); idx_src += 2, idx_dst++) {
    ...
    ESP_LOGD(TAG, "mjd_hexstring_to_string() param_ptr_input  len=%i (HEXDUMP)", strlen(param_ptr_input));
    ESP_LOG_BUFFER_HEXDUMP(TAG, param_ptr_input, strlen(param_ptr_input) + 1, ESP_LOG_DEBUG);  // +1 to see the \0
    ESP_LOGD(TAG, "mjd_hexstring_to_string() param_ptr_output len=%i (HEXDUMP)", strlen(param_ptr_output));
    ESP_LOG_BUFFER_HEXDUMP(TAG, param_ptr_output, strlen(param_ptr_output) + 1, ESP_LOG_DEBUG);  // +1 to see the \0
