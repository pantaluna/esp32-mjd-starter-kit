...
void gpio_setup_gpiomux_sensor_to_led() {
    ...
    gpio_matrix_in(MY_SENSOR_DATA_GPIO_NUM, SIG_IN_FUNC224_IDX, false);
...
void sensor_task(void *pvParameter) {
    ...
    ESP_LOGW(TAG, "  @doc The pin stays HIGH for at least 3 seconds when a movement is detected (depends on the Time Decay potmeter setting)");

    ESP_LOGW(TAG, "  @important WAIT 60 seconds so the PIR Sensor can calibrate itself!");
