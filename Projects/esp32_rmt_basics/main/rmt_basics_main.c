...
void main_task(void *pvParameter)
     ...
     *
     * Test the values of 1, 10, 100 and validate them using a logic analyzer. One tick is 50nanoseconds (see section rmt_config()).
     */
    rmt_item32_t items[] =
        {
            {
    ...
    for (uint8_t i = 0; i < nbr_of_items; i++)
        ...
        ESP_LOGD(TAG, "  %3i :: [lvl0] %4u - %4i (%6.1f ns) | [lvl1] %4u - %4i (%6.1f ns) | value uint32: %u", i,
                ptr_tmp_items->level0, ptr_tmp_items->duration0,
                ptr_tmp_items->duration0 * MY_RMT_ONE_TICK_DURATION_NANOSEC, ptr_tmp_items->level1,
                ptr_tmp_items->duration1, ptr_tmp_items->duration1 * MY_RMT_ONE_TICK_DURATION_NANOSEC,
                ptr_tmp_items->val);
        ptr_tmp_items++; // Advance ptr
        }

    f_retval = rmt_write_items(rmt_cfg_ch0.channel, items, nbr_of_items,
    true);
    if (f_retval != ESP_OK)
        {
